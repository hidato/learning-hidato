...
def random_neighbor(width, height, index):
    key = (width, height)
    if key not in cache:
...
class Model(object):
    ...
    def undo_move(self, index_value):
...
def generate_puzzle(width, height):
    ...
    grid = state.get_grid()
    # random_erase_grid(width, height, grid)
    return grid
...
def display(width, height, grid):
    for y in range(height):
        ...
        for x in range(width):
            i = y * width + x
            item = grid[i]
            item = '' if item == undefined_cell else str(item)
            row.append('%s|' % item.rjust(2, ' '))
        print(''.join(row))
    print(''.join(['+'] + ['--+' for _ in range(width)]))


def random_erase_grid(width, height, grid, erase_cnt=-1):
    size = width * height
    first_index = grid.index(1)
    last_index = grid.index(size)
    if erase_cnt == -1:
        erase_cnt = random.randint(1, size-2)
    for i in random.sample(range(size), erase_cnt):
        grid[i] = undefined_cell
    grid[first_index] = 1
